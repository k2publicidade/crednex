...
export interface Db {version:number;users:Account[];sessions:Record<string,{userId:string;expires:number}>;rules:Rules;ledger:Entry[];contracts:Contract[];deposits:any[];withdrawals:any[];tickets:any[];spins:any[];audit:any[];salaryMonths:string[]}
export const id=()=>crypto.randomUUID()
export const emptyDb=():Db=>({version:1,users:[],sessions:{},rules:structuredClone(DEFAULT_RULES),ledger:[],contracts:[],deposits:[],withdrawals:[],tickets:[],spins:[],audit:[],salaryMonths:[]})
...
export function subscribe(db:Db,userId:string,planId:string,cents:number,wallet:Wallet,at=new Date()) {
  ...
  const first=!db.contracts.some(c=>c.userId===userId)
  ...
  if(first&&user?.sponsorId&&eligible(db,user.sponsorId))spin(db,user.sponsorId,`activation:${userId}`)
...
export function draw(db:Db,userId:string,random=(max:number)=>crypto.randomInt(max)) {
  ...
  const token=db.spins.find(s=>s.userId===userId&&s.status==='AVAILABLE');if(!token)throw new Error('Nenhum giro disponível')
