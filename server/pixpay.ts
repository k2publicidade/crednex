...
export function pixPayConfig() {
  ...
  const baseUrl = (process.env.PIXPAY_BASE_URL?.trim() || 'https://webhookxxx.2pp.online').replace(/\/$/, '')
  let endpoint: URL
  try { endpoint = new URL(baseUrl) } catch { throw new PixPayConfigurationError('URL da API 2PP inválida') }
  if (endpoint.protocol !== 'https:' || endpoint.username || endpoint.password || endpoint.search || endpoint.hash) {
    throw new PixPayConfigurationError('A API 2PP exige uma URL HTTPS sem credenciais embutidas')
  }
  return {
    ...
    baseUrl,
    webhookToken,
    webhookUrl: `${publicUrl()}/api/webhooks/2pp?token=${encodeURIComponent(webhookToken)}`,
  }
}
...
export async function createPixPayTransaction(input: {
  ...
  if (input.depositId) webhookUrl.searchParams.set('depositId', input.depositId)
