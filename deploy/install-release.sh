...
for attempt in {1..15}; do
  if curl --fail --silent http://127.0.0.1:4020/api/health >/dev/null &&
    ...
    test "$(curl --silent --output /dev/null --write-out '%{http_code}' -X POST -H 'Content-Type: application/json' -d '{}' http://127.0.0.1:4020/api/admin/users/deployment-probe/balance)" = 401 &&
    test "$(curl --silent --output /dev/null --write-out '%{http_code}' -X POST -H 'Content-Type: application/json' -d '{}' http://127.0.0.1:4020/api/admin/users/deployment-probe/password)" = 401 &&
    test "$(curl --silent --output /dev/null --write-out '%{http_code}' -X PATCH -H 'Content-Type: application/json' -d '{}' http://127.0.0.1:4020/api/admin/support)" = 401; then
    healthy=1; break
  fi
  sleep 1
done
