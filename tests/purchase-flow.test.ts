...
test('PIX confirmado compra plano e credita três níveis uma vez; pausa exige confirmação explícita',async()=>{
 ...
 try{
  ...
  const before=await Promise.all(users.slice(0,3).map(async u=>(await call('/state',u.token)).body.balances.earnings))
  const purchase={planId:'C-1',amount:40,wallet:'deposit',requestId:'final-purchase-0001'}
  ...
  for(let i=0;i<3;i++){const state=(await call('/state',users[i].token)).body;assert.equal(state.balances.earnings-before[i],[80,120,400][i]);assert.equal(state.ledger.filter((e:any)=>e.key.startsWith(results[0].body.id+':level:')).length,1)}
  ...
  assert.equal((await call('/admin/rules',admin.token,{...rules,confirmed:false,pauseConfirmation:'PAUSAR COMPRAS'},'PATCH')).status,200)
