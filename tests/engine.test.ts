...
test('Credcofre: capital depositado retorna ao saldo e ganhos ficam em rendimentos',()=>{const db=fixture(),before=balance(db,'a','deposit'),c=subscribe(db,'a','CREDCOFRE',2500,'deposit',start);accrue(db,new Date(+start+DAY));assert.equal(balance(db,'a','vault'),2500);assert.equal(balance(db,'a','earnings'),50);assert.throws(()=>withdraw(db,'a','vault',2500,'pix',start),/Somente/);redeem(db,'a',c.id,new Date(+start+DAY+1000));accrue(db,new Date(+start+10*DAY));assert.equal(balance(db,'a','vault'),0);assert.equal(balance(db,'a','deposit'),before);assert.equal(balance(db,'a','earnings'),50);assert.throws(()=>withdraw(db,'a','earnings',50,'pix',new Date(+start+DAY+1000)),/pacote ativo/);assert.throws(()=>redeem(db,'a',c.id))})
