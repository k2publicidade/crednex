...
import {emptyDb,entry,balance,subscribe,accrue,redeem,withdraw,settleWithdrawal,draw,salary,network} from '../server/engine.js'
...
test('ciclo simples: 35 créditos, principal devolvido, sem duplicação nem juros extras',()=>{const db=fixture(),before=balance(db,'a','deposit');const c=subscribe(db,'a','C-1',10000,'deposit',start);assert.equal(accrue(db,new Date(+start+DAY-1)),0);assert.equal(accrue(db,new Date(+start+DAY)),1);assert.equal(balance(db,'a','earnings'),800);accrue(db,new Date(+start+90*DAY));assert.equal(c.paidDays,35);assert.equal(c.status,'CLOSED');assert.equal(balance(db,'a','earnings'),38000);assert.equal(balance(db,'a','deposit'),before-10000);assert.equal(accrue(db,new Date(+start+91*DAY)),0)})
test('NEX é fixo, 4% por 50 dias, capital conforme configuração capturada',()=>{const db=fixture();assert.throws(()=>subscribe(db,'a','NEX-N1',5100,'deposit',start));db.rules.returnPrincipal=true;const before=balance(db,'a','deposit');const c=subscribe(db,'a','NEX-N1',5000,'deposit',start);db.rules.returnPrincipal=false;accrue(db,new Date(+start+51*DAY));assert.equal(balance(db,'a','earnings'),15000);assert.equal(balance(db,'a','deposit'),before-5000);assert.equal(c.paidDays,50)})
...
test('horários de saque em São Paulo: fronteiras e fins de semana',()=>{assert.equal(withdrawalOpen('earnings',new Date('2026-09-04T14:59:59Z')),false);assert.equal(withdrawalOpen('earnings',new Date('2026-09-04T15:00:00Z')),true);assert.equal(withdrawalOpen('earnings',new Date('2026-09-04T20:59:59Z')),true);assert.equal(withdrawalOpen('earnings',new Date('2026-09-04T21:00:00Z')),false);assert.equal(withdrawalOpen('earnings',new Date('2026-09-05T15:00:00Z')),false);assert.equal(withdrawalOpen('vault',new Date('2026-09-06T15:00:00Z')),true)})
test('Credcofre: resgate encerra rendimento, libera principal e preserva histórico',()=>{const db=fixture(),c=subscribe(db,'a','CREDCOFRE',2500,'deposit',start);accrue(db,new Date(+start+DAY));assert.equal(balance(db,'a','vault'),2550);assert.throws(()=>withdraw(db,'a','vault',2500,'pix',new Date('2026-09-06T15:00:00Z')));redeem(db,'a',c.id,new Date(+start+DAY+1000));accrue(db,new Date(+start+10*DAY));assert.equal(balance(db,'a','vault'),2550);const w=withdraw(db,'a','vault',2500,'pix',new Date('2026-09-06T15:00:00Z'));assert.equal(w.net,2250);assert.equal(w.fee,250);assert.equal(balance(db,'a','vault'),50);assert.throws(()=>redeem(db,'a',c.id))})
test('reserva impede gasto duplo; recusa devolve saldo apenas uma vez',()=>{const db=fixture();entry(db,'a','earnings',10000,'earn','Teste');const w=withdraw(db,'a','earnings',9000,'pix',start);assert.throws(()=>withdraw(db,'a','earnings',9000,'pix',start));settleWithdrawal(db,w.id,'REJECTED','Chave incorreta');assert.equal(balance(db,'a','earnings'),10000);assert.throws(()=>settleWithdrawal(db,w.id,'REJECTED','Outra'));assert.equal(balance(db,'a','earnings'),10000)})
test('comissões de 10%, 3%, 2% em três níveis e giro por primeira ativação',()=>{const db=fixture();subscribe(db,'a','C-1',2500,'deposit',start);for(const [id,sponsorId]of [['b','a'],['c','b'],['d','c']]){db.users.push({...db.users[0],id,username:id,inviteCode:id,sponsorId});entry(db,id,'deposit',100000,`fund:${id}`,'Teste');subscribe(db,id,'C-1',10000,'deposit',start)}const last=db.contracts.at(-1)!;const entries=db.ledger.filter(e=>e.key.startsWith(last.id+':level'));assert.deepEqual(entries.map(e=>[e.userId,e.cents]),[['c',1000],['b',300],['a',200]]);const before=db.spins.length;subscribe(db,'d','C-1',2500,'deposit',start);assert.equal(db.spins.length,before);assert.equal(network(db,'a').length,3)})
...
test('limite por usuário e plano bloqueia sem débito e libera vaga no encerramento',()=>{
  ...
  subscribe(db,'a','C-1',2500,'deposit',new Date(+start+35*DAY))
