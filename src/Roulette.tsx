...
export function Roulette({prizes,spins,draw,refresh}:{prizes:Prize[];spins:number;draw:()=>Promise<{prize:Prize}>;refresh:()=>Promise<void>}) {
  ...
  return <section className="luck-experience" aria-label="Roleta da sorte" aria-busy={busy}>
    ...
    <div className="luck-controls"><span className="luck-credits"><Gift size={16}/><strong>{spins}</strong> {spins===1?'giro disponível':'giros disponíveis'}</span>
      ...
      {!prizes.length?<p>Prêmios em preparação. Seus giros ficam guardados.</p>:!spins?<p>Você ainda não tem giros. Cada reinvestimento com saldo de rendimentos libera um giro.</p>:<p><ShieldCheck size={13}/> Resultado confirmado com segurança.</p>}
    </div>
    <details className="luck-rules"><summary>Como funciona</summary><p>Cada reinvestimento com saldo de rendimentos libera um giro. Os prêmios são sorteados de forma segura e registrados no seu histórico.</p></details>
  </section>
}
