...
export function PlanCatalog({plans,rules,save,readOnly}:{plans:Plan[];rules:Rules;save:(body:Record<string,unknown>,id?:string)=>Promise<void>;readOnly?:boolean}) {
 ...
 return <section className="plan-catalog"><div className="section-heading"><div><h2>Planos disponíveis para contratação</h2><p>Cadastre os produtos oferecidos aos participantes. Somente planos ativos aparecem para novas compras.</p></div>{!readOnly&&<button className="primary" onClick={()=>edit(null)} disabled={busy}><Plus size={18}/>Cadastrar plano</button>}</div>
 <div className="info-strip">Comissões: 10%, 3% e 2% nos três níveis elegíveis, sobre {rules.commissionBase==='deposit'?'a aplicação confirmada':'os rendimentos'}. Comissões entram na Carteira de Indicações; o rendimento dos ciclos fica travado e é liberado no encerramento. Saques na janela 12h–18h (seg–sex).</div>
